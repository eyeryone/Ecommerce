...
import com.liuser.ecommerce.offline.udf.CityRatioUDAF
...
object AreaCountApp {
    def main(args: Array[String]): Unit = {
        ...
        //按地区商品进行分组 count
        sparkSession.sql("select area,click_product_id ,count(*) clickcount, " +
            "city_ratio(city_name) city_remark from v_action_city group by  area,click_product_id "
        ).createOrReplaceTempView("v_area_product_clickcount")
